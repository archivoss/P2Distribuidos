            ...
            while (true) {
                ...
                if (textoFormateado.charAt(0) == '@') {
                    ...
                    for (HiloDeCliente hilo : lista) {
                        if (hilo.getNombreUsuario().equals(nombreDestinatario)) {
                            try {
                                dataOutput.writeUTF(
                                        "[Mensaje privado enviado a (" + nombreDestinatario + ")]: " + mensaje);
                                mensajesHistorial
                                        .add("[Mensaje privado enviado a (" + nombreDestinatario + ")]: " + mensaje);
                ...
                if (textoFormateado.startsWith("@mensajeadministracion:")) {
                    try {
                        ...
                        List<String> gruposAdministracion = Arrays.asList("Medicos", "Admision", "Pabellon",
                                "Examenes");
                                            ...
                                            if (hilo.getNombreUsuario().equals(miembro.getNombreUsuario())) {
                                                try {
                                                    hilo.dataOutput.writeUTF(
                                                            "[Mensaje enviado a (Administración)]: " + mensaje);
                                                } catch (Exception e) {
                                                    e.printStackTrace();
                                                }
                                            }
                    ...
                    for (Grupo grupo : listaGrupos) {
                        for (HiloDeCliente miembro : grupo.getListaMiembros()) {
                            if (miembro.getNombreUsuario().equals(nombreUsuario)
                                    && !grupo.getNombreGrupo().equals("Auxiliar")) {
                                for (HiloDeCliente hilo : grupo.getListaMiembros()) {
                                    for (HiloDeCliente hiloAux : lista) {
                                        if (hilo.getNombreUsuario().equals(hiloAux.getNombreUsuario())) {
                                            dataOutput.writeUTF("[Mensaje del grupo (" + grupo.getNombreGrupo()
                                                    + ") por (" + hilo.getNombreUsuario() + ")]: " + textoFormateado);
                                            hiloAux.dataOutput.writeUTF("[Mensaje del grupo (" + grupo.getNombreGrupo()
                                                    + ") por (" + hilo.getNombreUsuario() + ")]: " + textoFormateado);
                                            grupo.setMensajes("[Mensaje del grupo (" + grupo.getNombreGrupo()
                                                    + ") por (" + hilo.getNombreUsuario() + ")]: " + textoFormateado);
                                        }
                                    }
                                }
                            ...
                            if (miembro.getNombreUsuario().equals(nombreUsuario)
                                    && grupo.getNombreGrupo().equals("Auxiliar")) {
                                for (HiloDeCliente hilo : lista) {
                                    hilo.dataOutput.writeUTF("[Mensaje desde el grupo (Auxiliar)]: " + textoFormateado);
                                }
                            }
                        }
                    }
                ...
                if (textoFormateado.startsWith("@todos:")) {
                    List<HiloDeCliente> lista = ServidorChat.getListaHilos();
                    String mensaje = textoFormateado.substring(7);
                    for (HiloDeCliente hilo : lista) {
